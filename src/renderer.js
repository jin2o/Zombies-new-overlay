...
function handleImageError(imgElement, uuid, playerName) {
    ...
    const newUrl = `https://crafatar.com/avatars/${uuid}?size=24&default=MHF_Steve&overlay&${timestamp}`;
    console.log(`Retrying image load for ${playerName} with URL: ${newUrl}`);
    
    setTimeout(() => {
        imgElement.src = newUrl;
        imgElement.onerror = function() {
            console.log(`Second attempt failed for ${playerName}, showing default head`);
            const defaultTimestamp = Date.now();
            this.src = `https://crafatar.com/avatars/8667ba71b85a4004af54457a9734eed7?size=24&default=MHF_Steve&overlay&${defaultTimestamp}`;
            this.onerror = null;
        };
    }, 500);
}
...
function updatePlayerDisplay() {
    ...
    sortedPlayers.forEach(player => {
        ...
        if (uuid) {
            const timestamp = Date.now();
            const playerHeadUrl = `https://crafatar.com/avatars/${uuid}?size=24&default=MHF_Steve&overlay&${timestamp}`;
            const imgId = `player-head-${player.replace(/\s+/g, '-')}-${timestamp}`;
            ...
            const timestamp = Date.now();
            const defaultHeadUrl = `https://crafatar.com/avatars/8667ba71b85a4004af54457a9734eed7?size=24&default=MHF_Steve&overlay&${timestamp}`;
            tableHtml += `<div class="table-player-name">
